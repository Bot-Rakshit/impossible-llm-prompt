...
import chess
from chess import Board, Color, Move, ROOK, parse_square
from chess.pgn import ChildNode, Game, GameNode


@dataclass
class _FallbackPuzzle:
    id: str
    game: Game
    cp: int
...
def _fallback_read(doc: dict[str, Any]) -> _FallbackPuzzle:
    board = Board(doc["fen"])
    ...
    return _FallbackPuzzle(doc["_id"], node.game(), int(doc["cp"]))
...
def _load_impl():
    try:
        ...
        cook_mod = mod
        if hasattr(mod, "cook") and hasattr(getattr(mod, "cook"), "attraction"):
            cook_mod = mod.cook
        util_mod = mod
        if hasattr(mod, "util") and hasattr(getattr(mod, "util"), "is_trapped"):
            util_mod = mod.util
        read_fn = getattr(mod, "read", _fallback_read)
        logger_obj = getattr(mod, "logger", logging.getLogger("solution"))
        return util_mod, cook_mod, read_fn, logger_obj
    except ModuleNotFoundError as exc:
        if exc.name != "solution":
            raise

    util_mod = importlib.import_module("util")
    cook_mod = importlib.import_module("cook")
    try:
        tagger_mod = importlib.import_module("tagger")
        read_fn = getattr(tagger_mod, "read", _fallback_read)
        logger_obj = getattr(tagger_mod, "logger", logging.getLogger("tagger"))
    except ModuleNotFoundError as exc:
        if exc.name != "tagger":
            raise
        read_fn = _fallback_read
        logger_obj = logging.getLogger("tagger")
    return util_mod, cook_mod, read_fn, logger_obj


try:
    util, cook, read, logger = _load_impl()
except ModuleNotFoundError as exc:
    raise ImportError(
        "No candidate implementation found. Provide either `solution.py` "
        "or legacy modules (`cook.py` and `util.py`, with optional `tagger.py`)."
    ) from exc


def make(id: str, fen: str, line: str):
